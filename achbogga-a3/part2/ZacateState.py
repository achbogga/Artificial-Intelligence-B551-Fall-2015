...
class Scorecard:
    ...
    def category_wise_exp_if_bonus_and_score(self, category, roll):
		s = 0;
		dice = roll.dice
		counts = [dice.count(i) for i in range(1,7)]
		if category in self.scorecard:
			print "Error: category already full!"
		if category in Scorecard.Numbers:
			s = counts[Scorecard.Numbers[category]-1] * Scorecard.Numbers[category]
		elif category == "pupusa de queso":
			s = 40 if sorted(dice) == [1,2,3,4,5] or sorted(dice) == [2,3,4,5,6] else 0
		elif category == "pupusa de frijol":
			s = 30 if (len(set([1,2,3,4]) - set(dice)) == 0 or len(set([2,3,4,5]) - set(dice)) == 0 or len(set([3,4,5,6]) - set(dice)) == 0) else 0
		elif category == "elote":
			...
			print "Error: unknown category"
		# check for bonus and add if it exists
		if not self.bonusflag and len(set(Scorecard.Numbers.keys()) - set(self.scorecard.keys())) == 0:
			self.bonusscore = 35 if sum([ self.scorecard[i] for i in Scorecard.Numbers ]) >= 63 else 0
			s+=35
		return s   
